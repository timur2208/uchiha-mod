...
public class MagicData implements INBTSerializable<CompoundTag> {
    ...
    @Override public CompoundTag serializeNBT() { CompoundTag tag = new CompoundTag(); tag.putFloat("CurrentMana", currentMana); tag.putFloat("MaxMana", maxMana); return tag; }
    @Override public void deserializeNBT(CompoundTag nbt) { if (nbt.contains("CurrentMana", Tag.TAG_FLOAT)) currentMana = nbt.getFloat("CurrentMana"); if (nbt.contains("MaxMana", Tag.TAG_FLOAT)) maxMana = nbt.getFloat("MaxMana"); }
}